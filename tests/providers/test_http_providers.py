...
def test_glm_intl_chat_completion_salvages_think_only_stream() -> None:
    credentials = ProviderCredentialRecord(
        provider="glm-intl",
        kind="browser_session",
        cookie="token=cookie-token",
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/api/v1/auths/":
            return httpx.Response(200, json={"id": "user-123", "name": "loki", "token": "bearer-123"})
        if request.url.path == "/api/models":
        ...
        if request.url.path == "/api/chat/completions":
            return httpx.Response(
                200,
                text=(
                    'data: {"type":"chat:completion","data":{"delta_content":"深","phase":"thinking"}}\n\n'
                    'data: {"type":"chat:completion","data":{"delta_content":"入","phase":"thinking"}}\n\n'
                    'data: {"type":"chat:completion","data":{"delta_content":"分","phase":"thinking"}}\n\n'
                    'data: {"type":"chat:completion","data":{"delta_content":"析","phase":"thinking"}}\n\n'
                    'data: {"type":"chat:completion","data":{"phase":"done","done":true}}\n\n'
                    "data: [DONE]\n\n"
                ),
                headers={"content-type": "text/event-stream"},
    ...
    assert "".join(client.iter_chat_completion_text(message="hello glm-intl", model="GLM-5.1")) == "深入分析"
    assert client.chat_completion(message="hello glm-intl", model="GLM-5.1") == "深入分析"


def test_glm_intl_api_client_accepts_list_style_models_payload() -> None:
    credentials = ProviderCredentialRecord(
        provider="glm-intl",
        kind="browser_session",
        cookie="token=cookie-token",
