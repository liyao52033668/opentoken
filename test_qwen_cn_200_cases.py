#!/usr/bin/env python3
"""Qwen-CN 200+ case E2E suite with real credentials and detailed report."""
...
MODEL = "algae/qwen-cn/Qwen3.5-Plus"
SECONDARY_MODEL = "algae/qwen-cn/Qwen3.5-Turbo"
...
def run_post(name: str, endpoint: str, payload: dict, *, category: str, timeout: float = 60.0) -> None:
    ...
    try:
        resp = httpx.post(f"{BASE_URL}{endpoint}", headers=HEADERS, json=payload, timeout=timeout)
...
def run_stream(name: str, endpoint: str, payload: dict, *, category: str, timeout: float = 60.0) -> None:
    ...
    try:
        chunks = []
        with httpx.stream("POST", f"{BASE_URL}{endpoint}", headers=HEADERS, json=payload, timeout=timeout) as resp:
...
def write_report() -> Path:
    report_path = Path("qwen_cn_200_cases_report.md")
    total = len(results)
    ...
    lines = []
    lines.append("# Qwen-CN 200+ Case E2E Report")
    lines.append("")
