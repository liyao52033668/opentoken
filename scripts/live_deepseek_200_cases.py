#!/usr/bin/env python3
"""DeepSeek 200+ case E2E suite with real credentials and detailed report."""
...
SECONDARY_MODEL = "algae/deepseek/deepseek-reasoner"
...
def restart_server() -> None:
    ...
    time.sleep(2)
    subprocess.Popen("uv run opentoken start >/tmp/algae-deepseek-case.log 2>&1", shell=True)
...
def write_report() -> Path:
    report_path = Path("deepseek_200_cases_report.md")
    total = len(results)
    ...
    lines = []
    lines.append("# DeepSeek 200+ Case E2E Report")
    lines.append("")
